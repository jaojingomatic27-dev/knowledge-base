...
## [2026-06-04 02:00] 清理 Temu、eBay、Promotions 推广邮件

- **输入命令**: "删除Temu" → "删除>2周eBay" → "整理推广标签邮件"
- **PROJECT_INDEX 变更**: 新增 5 个 mail 脚本
- **关键发现**:
  1. 孤立邮件（IMAP 无法删除）通过 MOVE 到垃圾邮件解决
  2. `X-GM-RAW` 只在非 UID SEARCH 下可用
  3. 累计清理 5,466 封，收件箱 7,248 → ~1,970（73%）
- **生成/修改的文件**:
  | 文件 | 说明 |
  |------|------|
  | `mail/code/delete_temu.py` 等 5 个 | 专项清理脚本 |
